...
public class EngineRegression {
 ...
 public static void main(String[]args)throws Exception{
  ...
  for(int i=0;i<modes.getLength();i++){var e=(Element)modes.item(i);var m=new Mode(e.getAttribute("NAME").toLowerCase(java.util.Locale.ROOT));m.setProperty("file",base.resolve("modes").resolve(e.getAttribute("FILE")).toString());if(e.hasAttribute("FILE_NAME_GLOB"))m.setProperty("filenameGlob",e.getAttribute("FILE_NAME_GLOB"));if(e.hasAttribute("FIRST_LINE_GLOB"))m.setProperty("firstlineGlob",e.getAttribute("FIRST_LINE_GLOB"));m.init();ModeProvider.instance.addMode(m);}
  ...
  for(int i=0;i<cases.getLength();i++){var c=(Element)cases.item(i);String mode=c.getAttribute("mode").toLowerCase(java.util.Locale.ROOT);String[] lines=c.getElementsByTagName("SOURCE").item(0).getTextContent().split("\n",-1);var tm=ModeProvider.instance.getMode(mode).getTokenMarker();TokenMarker.LineContext ctx=null;var tokens=new ArrayList<byte[]>();
  ...
  String[][] globTests={{"a.xml","xml"},{"a.svg","xml"},{"catalog","xml"},{"a.vh","verilog"},{"a.vhdl","vhdl"},{"a.hxx","c++"},{"a.hlsl","hlsl"},{"a.fx","javafx"},{"Makefile","makefile"},{"test.mk","makefile"},{"test.mak","makefile"},{"a.f","fortran"},{"a.fth","forth"},{"a.mm","objective-c"},{"a.vbs","vbscript"},{"a.bas","visualbasic"},{"a.cfc","coldfusion"},{"a.cfs","cfscript"},{"a.p","progress"},{"a.ssi","shtml"},{"a.ssc","s+"},{"a.d.ts","typescript"},{"a.d.mts","typescript"},{"a.d.cts","typescript"},{"x.mjs","javascript"},{"x.cts","typescript"},{"jsconfig.app.json","jsonc"},{"a.xslt","xsl"},{"test.ts","typescript"},{"x.tsx","tsx"},{"x.jsx","jsx"},{"x.mts","typescript"},{"x.cjs","javascript"},{"x.rs","rust"},{"build.gradle.kts","kotlin"},{"Package.swift","swift"},{"Cargo.toml","toml"},{"query.graphql","graphql"},{"Dockerfile","dockerfile"},{"Dockerfile.dev","dockerfile"},{"main.tf","hcl"},{"terraform.tfvars","hcl"},{"tsconfig.json","jsonc"},{"package.json","json"},{"data.json5","json5"},{"legacy.jsonp","javascript"},{"x.pyi","python"},{"config.yml","yaml"}};
